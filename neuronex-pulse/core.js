...
let isListeningLoop = false; // Estado de escucha continua
...
function initVoiceInterface() {
    ...
    recognition.continuous = false; // Reiniciamos manualmente para mejor control
    ...
    micBtn.addEventListener('click', () => {
        if (isListeningLoop) {
            ...
            micStatus.textContent = "Micrófono Pausado";
        } else {
            // Iniciar escucha continua
            isListeningLoop = true;
            try { recognition.start(); } catch(e) {}
            micBtn.classList.add('mic-listening');
            micStatus.textContent = "Escucha Activa (Bucle)";
        }
    });

    recognition.onstart = () => {
        micBtn.classList.add('mic-listening');
        if (!micStatus.textContent.includes("Bucle")) micStatus.textContent = "Escuchando...";
        setEmotion('ANALIZANDO');
    };
    ...
    recognition.onend = () => {
        ...
        if (isListeningLoop) {
            setTimeout(() => {
                try { recognition.start(); } catch(e) { console.log("Reinicio voz ignorado"); }
            }, 500);
        } else {
            micBtn.classList.remove('mic-listening');
    ...
    recognition.onerror = (event) => {
        ...
        if (event.error === 'not-allowed' || event.error === 'service-not-allowed') {
            isListeningLoop = false;
            micStatus.textContent = "Error: Acceso denegado";
            micBtn.classList.remove('mic-listening');
        }
    };
